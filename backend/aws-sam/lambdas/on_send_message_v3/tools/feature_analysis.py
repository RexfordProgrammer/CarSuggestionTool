...
# Bedrock client for a single JSON-only call
...
def _llm_detect_flags(history: List[Dict], flags: List[str]) -> Dict[str, bool]:
    """Ask the model directly for JSON booleans per flag."""
    convo = []
    for m in history:
    ...
    system = [{
        "text": (
            ...
            "and whose values are booleans (true/false) indicating if each feature was mentioned or implied. "
